...
function createPetElement(pet) {
  ...
  petDiv.innerHTML = `
        ...
        ${pet instanceof Cat ? '<button class="scratch">Scratch</button>' : ''}
        <p class="message"></p>
    `;
  ...
  // If the pet is a Cat, add event listener for the scratch button
  if (pet instanceof Cat) {
    petDiv.querySelector('.scratch').addEventListener('click', () => {
      const message = pet.scratch();
      updatePetUI(petDiv, pet, message);
    });
  }
